import { connectContract, stringToBytes32 } from "./helpers";

(async () => {
  try {
    const { contract, provider, wallet } = await connectContract();
    ...
    while (isMinted === false) {
      ...
      process.stdout.write(
        `\x1B[2K\r${
          isMinted ? "Minted." : `Missed(It's already ${receipt.blockNumber}).`
        }\n`
      );

      targetBlockNumber = receipt.blockNumber - blockNumber;

      if (isMinted) process.exit(0);
